...
for %%n in (1, 2, 5, 10, 20, 50, 100, 200, 500, 1000, 2000, 5000, 10000, 20000, 50000, 100000) do (
  ...
  hermes-cli -o data\latency -d 60 --experiment run=latency_vs_frequency trial=!counter! --config_file test_latency.yml

  python latency.py !counter! %%n 1
  rmdir /s /q ".\data\latency\run_latency_vs_frequency\trial_!counter!"
...
for %%n in (10, 20, 50, 100, 200, 500, 1000, 2000, 5000, 10000, 20000, 50000, 100000, 200000, 500000, 1000000) do (
  ...
  echo Starting experiment !counter!...
  hermes-cli -o data\latency -d 60 --experiment run=latency_vs_msgsize trial=!counter! --config_file test_latency.yml

  python latency.py !counter! %%n 0
  rmdir /s /q ".\data\latency\run_latency_vs_msgsize\trial_!counter!"
