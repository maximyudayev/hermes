...
## Environment
Recreate the environment by `conda env create --file environment.yml` on each networked PC that will connect to sensors and broker data in the setup in any capacity (i.e. fixed and wearable PC).

With `aidwear` conda environment active, install missing PyPi packages `pip install PyQt6 opencv-python`.
